...
(* This function receives a non expr and returns a non-located typed_expr *)
let rec type_expr classname_str classesEnv varEnv expr = 
	let type_unop u e = 
		let ne = type_expr classname_str classesEnv varEnv e in 
	...
	and type_binop b e1 e2 =
		let ne1 = type_expr classname_str  classesEnv varEnv e1
		and ne2 = type_expr classname_str  classesEnv varEnv e2
	...
	and type_condition i t e = 
		let ni = type_expr classname_str  classesEnv varEnv i and
		nt = type_expr classname_str  classesEnv varEnv t and
		ne = type_expr classname_str  classesEnv varEnv e in
	...
	and type_method_call e m args = 
		let rec do_l = function 
			| [] -> []
			| t::q -> (Located.mk_elem (type_expr classname_str  classesEnv varEnv t) 
				(Located.loc_of t))::(do_l q)
		and type_args = function
			| [] -> []
			| t::q -> (type_of_expr (Located.elem_of t))::(type_args q)
		and ne = type_expr classname_str  classesEnv varEnv e
	...
	and type_local_variable c v ve e =
		let nve = type_expr classname_str  classesEnv varEnv ve
		and classname_type = type_of_classname classesEnv (Located.elem_of c) (Located.loc_of c) 
		in let ne = type_expr classname_str  classesEnv ({
	...
	and type_attr_affect s e =
		...
		let ne = type_expr classname_str  classesEnv varEnv e
	...
	and type_this () = 
		...
		if Option.is_none classname_str then
	...
	| Instanceof (e, c) -> TypedInstanceof(Located.mk_elem (type_expr classname_str classesEnv varEnv e) 
...
let rec type_attr_or_method_list classesEnv currentClassEnv l =
	...
	let type_method c s params e static = 
		...
		in let ne = type_expr (Some currentClassEnv.name) classesEnv ((parse_attributes currentClassEnv.attributes)@params_vartypes) e
		in
	...
	and type_attr_with_value c s e static =
		...
		let ne = type_expr (Some currentClassEnv.name) classesEnv [] e
		in let tne = (type_of_expr ne)
		in 
...
let type_structure_tree tree = 
	...
	in let rec type_rec_structure_tree sub_tree =
		...
		let type_structure = function
			| Expr e -> TypedExpr (Located.mk_elem (type_expr None classesEnv [] e) (Located.loc_of e))
